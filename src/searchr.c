...
int LevelControl[3];
static int ValueFilter( int value );

void SetRandomNumber( void )
{
  int blur = abs( LevelControl[1] );
  if ( blur != 0 ) {
    RandomNumber = ( rand() % blur ) - blur / 2;
  } else {
    RandomNumber = 0;
  }
}
int ValueFilter( int value )
{
  int limit = LevelControl[0] + RandomNumber;
  int step = ( LevelControl[2] > 0 ) ? LevelControl[2] : 1234567;
  int n = ( ( value - limit ) % step ) * 2;
  if( value <= limit ) {
    return value;
  } else if( n <= step && step != 1234567 ) {
    return value;
  } else {
    return value - n;
  }
